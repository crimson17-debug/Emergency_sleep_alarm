// Arduino combined code-anti alarm sleep code plus gsm module code
...
void setup() {
  ...
  Serial.println("Press button or use commands: s (SMS), r (Receive), c (Call)");
}
...
void readSensorState() {
  ...
  if(currentMillis - previousSensorMillis > intervalSensor) {
    ...
    if (sensorState == LOW && isLongPressActive && sensorOutDuration >= minSensorDuration2) {
      ...
      // No blocking delay here, replacing with non-blocking buzz pattern
      // The buzzer will be turned off on button release
    }
...
// Make a phone call
void callNumber() {
  Serial.println("Calling number: " + phoneNumber);
  sim.print(F("ATD"));
